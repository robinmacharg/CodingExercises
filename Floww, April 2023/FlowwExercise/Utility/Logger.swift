...
// Configured like so:
//
//     Logger.shared.configuration = FWLogConfig(
//         // Only log warnings and above
//         logLevel: FWLogLevel.warning,
//         display:
...
//
// swiftlint:disable comma large_tuple no_space_in_method_call line_length colon opening_brace statement_position
 ...
 * these off.
 */
public enum FWLogLevel: Int, CaseIterable {
    case debug     = 0
    case info      = 1
    case event     = 2
    case warning   = 3
 ...
 */
public protocol FWLogConfigurationProtocol {
    var logLevel: FWLogLevel { get }
    var filter: ((String) -> (Bool))? { get }
    var display: [FWLogLevel : FWLogLevelDisplay] { get }
}
 ...
 */
public struct FWLogConfig : FWLogConfigurationProtocol {
    public var logLevel: FWLogLevel = .none
    public var filter: ((String) -> (Bool))?

    // Default display configuration
    public var display: [FWLogLevel : FWLogLevelDisplay] = [
    ...
    public init(
        logLevel: FWLogLevel = .none,
        filter: ((String) -> (Bool))? = nil,
        display: FWLogLevelDisplay...)
    {
        ...
        for level in display {
            switch level {
            // Override for all levels.  The order that FWLogLevelDisplays are provided
...
public final class Logger {
    ...
    // How much to indent the log level ("FW:INFO" etc.) by
    private static var levelIndent = 0

    public var configuration: FWLogConfigurationProtocol

    static let shared = Logger()
    private init() {
        configuration = FWLogConfig()

        // Set the indent level based on the possible values it can take
        Logger.levelIndent = (FWLogLevel.allCases.map { $0.name.count }.max() ?? 0) + 4 // +4 to allow for space and arrow.
    }
     ...
     */
    private static func symbolString(_ logLevel: FWLogLevel, _ force: Bool) -> String {
        var messageString = ""
        switch logLevel {
