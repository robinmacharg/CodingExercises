...
// Configured like so:
//
//     Logger.shared.configuration = PLLogConfig(
//         // Only log warnings and above
//         logLevel: PLLogLevel.warning,
//         display:
 ...
 * these off.
 */
public enum PLLogLevel: Int, CaseIterable {
    case debug     = 0
    case info      = 1
    case event     = 2
    case warning   = 3
 ...
 */
public protocol PLLogConfigurationProtocol {
    var logLevel: PLLogLevel { get }
    var filter: ((String) -> (Bool))? { get }
    var display: [PLLogLevel : PLLogLevelDisplay] { get }
}
 ...
 */
public struct PLLogConfig : PLLogConfigurationProtocol {
    public var logLevel: PLLogLevel = .none
    public var filter: ((String) -> (Bool))?

    // Default display configuration
    public var display: [PLLogLevel : PLLogLevelDisplay] = [
    ...
    public init(
        logLevel: PLLogLevel = .none,
        filter: ((String) -> (Bool))? = nil,
        display: PLLogLevelDisplay...)
    {
        ...
        for level in display {
            switch level {
            // Override for all levels.  The order that PLLogLevelDisplays are provided
...
public final class Logger {
    ...
    // How much to indent the log level ("PL:INFO" etc.) by
    private static var levelIndent = 0

    public var configuration: PLLogConfigurationProtocol

    static let shared = Logger()
    private init() {
        configuration = PLLogConfig()

        // Set the indent level based on the possible values it can take
        Logger.levelIndent = (PLLogLevel.allCases.map { $0.name.count }.max() ?? 0) + 4 // +4 to allow for space and arrow.
    }
     ...
     */
    private static func symbolString(_ logLevel: PLLogLevel, _ force: Bool) -> String {
        var messageString = ""
        switch logLevel {
